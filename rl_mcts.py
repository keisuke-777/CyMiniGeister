...
# 価値が最も高い行動を選択(player)
def high_value_action(model):
    def high_value_action(state):
        legal_actions = state.legal_actions()  # 合法手のリスト

        # 合法手で最も価値の高いインデックスを求める
        value_action = predict(model, state)
        max_index = np.argmax(value_action[0])
