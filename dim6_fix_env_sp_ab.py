...
def kei_neat(cycle: int, file_pass: str, benchmark: list) -> None:
    # 適応度の高い個体をn個体取得
    n = NUM_OF_ELITES
    ...
    # 総当たりによって生成した個体を評価
    new_fitness_list = vs_benchmark(benchmark, new_eval_lists)
...
def vs_benchmark(benchmark: list, params_list: list) -> list:
    length = len(params_list)
    score_list = [0] * length

    for i in range(length):
        result = 0
        for bench_params in benchmark:
            result += buttle_double(params_list[i], bench_params)
        score_list[i] = result
...
if __name__ == "__main__":
    file_pass = "csv_data/dim6/FixSpuriousSearch/" + str(datetime.date.today()) + "/"

    # 1.ベンチマークとなる評価関数をランダムに100個生成
    benchmark = create_many_eval_func(NUM_OF_SAVES, STANDARD_WINNING_RATE)

    # 2.評価関数をランダムに100個生成
    eval_lists = create_many_eval_func(NUM_OF_SAVES, STANDARD_WINNING_RATE)

    # 3.生成した評価関数をベンチマークと対戦させる
    fitness_list = vs_benchmark(benchmark, eval_lists)
    ...
    for i in range(10000):
        ...
        kei_neat(i, file_pass, benchmark)
