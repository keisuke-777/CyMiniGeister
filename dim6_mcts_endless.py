...
def buttle_double(params: list, enemy_params: list, enemy_agent=None) -> float:
    ...
    while True:
        ...
        # 行動の取得
        if state.is_first_player():
            action = spurious_mcts_action(state, estimate.fst_est_val_and_coo, 300)
            estimate.double_update_from_action(action, state, True)
        else:
            if enemy_agent == None:
                action = spurious_mcts_action(state, estimate.sec_est_val_and_coo, 300)
            else:
                action = enemy_agent(state)
            estimate.double_update_from_action(action, state, False)
        state = state.next(action)
...
if __name__ == "__main__":
    file_pass = "csv_data/dim6/SpuriousMCTS/" + str(datetime.date.today()) + "/"
