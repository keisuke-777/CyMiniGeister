# SpuriousSearch、永久、進化計算
import random, datetime, copy, csv, os, sys, itertools
...
# iwasaki_evolからの変更点
# 制約を満たすようなパラメータを生成
def generate_rdm_params(min: int = MIN, max: int = MAX, width: int = WIDTH) -> list:
    rdm_params = []
    rdm_params.append(BLUE_TABLE)  # 青駒テーブル
    rdm_params.append(RED_TABLE)  # 赤駒テーブル
    # 隣接してきた駒,逃げた駒,ゴールに近づいてきた駒
    est_table = [random.uniform(-EST_EVAL_RANGE, EST_EVAL_RANGE) for _ in range(3)]
    est_table.extend([0, 0])
...
def league_match(params_list: list) -> list:
    ...
    for vs_tp in vs_tuples:
        result = buttle_double(params_list[vs_tp[0]], params_list[vs_tp[1]])
...
def evaluation_against_elites(elites: list, new_eval_lists: list) -> list:
    score_list = [0] * 100
    for index, new_params in enumerate(new_eval_lists):
        for elite in elites:
            score_list[index] += evaluate_elite(new_params, elite)
        score_list[index] = score_list[index] / len(elites)
    return score_list
...
# 評価関数->estの3要素
def create_three_est_from_ev_func(ev_func: list) -> list:
    three_est = ev_func[2][:3]
    return three_est


def kei_neat(cycle: int, file_pass: str) -> None:
    # 適応度の高い個体をn個体取得
    n = NUM_OF_ELITES
    _, eval_lists = read_parent(cycle, file_pass)
    three_est_eval_lists = eval_lists[:n]

    # csvに保存されているのはestの3要素だけなので評価関数を復元する
    surviving_evals = [0] * len(three_est_eval_lists)
    for i, three_est in enumerate(three_est_eval_lists):
        surviving_evals[i] = create_ev_func_from_three_est(three_est)
    ...
    # 生成した個体を評価（先代の優秀なk個体と対戦）
    # k = 3
    # parent_elites = surviving_evals[:k]
    # new_fitness_list = evaluation_against_elites(parent_elites, new_eval_lists)

    # 総当たりによって生成した個体を評価
    new_fitness_list = league_match(new_eval_lists)

    # csvに出力する情報を削減するため、評価関数をestの3要素に圧縮
    new_three_ests = [0] * len(new_eval_lists)
    for i, ev_func in enumerate(new_eval_lists):
        new_three_ests[i] = create_three_est_from_ev_func(ev_func)

    # fitnessが高い順にソート
    zip_lists = zip(new_fitness_list, new_three_ests)
...
if __name__ == "__main__":
    file_pass = "csv_data/SpuriousSearch/xNEAT" + str(datetime.date.today()) + "/"

    # 1.評価関数をランダムに100個生成
    eval_lists = create_many_eval_func(NUM_OF_SAVES, STANDARD_WINNING_RATE)

    # 2.生成した評価関数でリーグ戦をさせて勝率を計測（これを適応度とする）
    fitness_list = league_match(eval_lists)

    # csvに出力する情報を削減するため、評価関数をestの3要素に圧縮
    three_ests = [0] * len(eval_lists)
    for i, ev_func in enumerate(eval_lists):
        three_ests[i] = create_three_est_from_ev_func(ev_func)

    # fitnessが高い順にソート
    zip_lists = zip(fitness_list, three_ests)
    zip_sort = sorted(zip_lists, reverse=True)
    sorted_fit, sorted_three_ests = zip(*zip_sort)
